...
class CrystalDataScaler:
    ...
    def transform(self, g: dgl.DGLGraph) -> dgl.DGLGraph:
        ...
        g.ndata["total_energy"] = (g.ndata["total_energy"] - self.energy_mean.to(g.device)) / self.energy_std.to(g.device)
        g.ndata["stress_tensor_flat"] = (g.ndata["stress_tensor_flat"] - self.stress_mean.to(g.device)) / self.stress_std.to(g.device)
        return g
...
class CrystalDecoder(nn.Module):
    ...
    def forward(self, z: torch.Tensor, g: dgl.DGLGraph, edge_types: torch.Tensor, node_emb: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        edge_input = torch.cat([z_src, z_dst], dim=1) if len(z_src) > 0 else torch.zeros((0, self.hidden_dim * 4), device=z.device)
        recon_edge_feats = self.edge_decoder(edge_input)

        # 能量/应力预测
        lattice_feat = g.graph_attr['lattice'].repeat(z.shape[0], 1) if 'lattice' in g.graph_attr else torch.zeros(z.shape[0], 6, device=z.device)
...
try:
    ...
    for k in checkpoint_state_dict.keys():
        ...
        if key in model_state_dict and checkpoint_state_dict[k].shape == model_state_dict[key].shape:
            ...
            print(f"跳过不匹配的参数: {k} (checkpoint shape: {checkpoint_state_dict[k].shape if k in checkpoint_state_dict else 'N/A'}, model shape: {model_state_dict.get(key, torch.tensor([])).shape})")
    ...
    if 'scaler' in checkpoint:
        if isinstance(checkpoint['scaler'], CrystalDataScaler):
        ...
        elif isinstance(checkpoint['scaler'], dict):
    ...
    print(f"✅ RGCN-VAE模型加载成功（兼容模式）：{MODEL_PATH}")
    print(f"📌 加载的模型训练至Epoch {checkpoint.get('epoch', '未知')}")
    print(f"📌 成功加载 {len(matched_params)}/{len(checkpoint_state_dict)} 个参数")
except FileNotFoundError:
    raise RuntimeError(f"RGCN模型文件不存在：{MODEL_PATH}")
except Exception as e:
    raise RuntimeError(f"RGCN模型加载失败：{str(e)}") from e
...
def save_crystal_to_cif(structure_dict: Dict, filename: Path):
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(f"# Generated by RGCN-RGAG on {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("data_crystal_structure\n")
        f.write("_audit_creation_method 'RGCN-RGAG structure generation'\n")
...
def calculate_energy_and_stress(g: dgl.DGLGraph, lattice_params: List[float]) -> Tuple[float, np.ndarray]:
    ...
    with torch.enable_grad():
        ...
        stress_flat = -grad_energy[:6] * 10.0
        ...
        stress_tensor[0, 1] = stress_tensor[1, 0] = stress_flat[3].item() if stress_flat[3].numel() > 0 else random.uniform(-5, 5)
        stress_tensor[0, 2] = stress_tensor[2, 0] = stress_flat[4].item() if stress_flat[4].numel() > 0 else random.uniform(-5, 5)
        stress_tensor[1, 2] = stress_tensor[2, 1] = stress_flat[5].item() if stress_flat[5].numel() > 0 else random.uniform(-5, 5)
...
def enforce_atom_constraints(coords: torch.Tensor, atomic_nums: List[int], lattice: Lattice) -> torch.Tensor:
    ...
    # 步骤1：先修正分数坐标（确保原子在晶胞内）
    frac_coords = lattice.get_fractional_coords(coords.cpu().numpy())
    frac_coords = np.mod(frac_coords, 1.0)
    coords = torch.tensor(lattice.get_cartesian_coords(frac_coords), dtype=torch.float32, device=device)

    # 步骤2：强化原子间距约束（分阶段推进，更严格）
    stages = [0.5, 0.6, MIN_ATOM_DISTANCE]  # 先宽松后严格，避免震荡
...
def realtime_optimize_single_structure(g: dgl.DGLGraph, atom_types: List[int], lattice: Lattice) -> Tuple[dgl.DGLGraph, Lattice]:
    ...
    frac_coords = np.mod(frac_coords, 1.0)
    coords = torch.tensor(optimized_lattice.get_cartesian_coords(frac_coords), dtype=torch.float32, device=coords.device)
    node_feats[:, 1:4] = coords
    g.ndata['feat'] = node_feats
...
def generate_lattice_params(similar_structures: List[Dict], atom_types: List[int], sg_cell_constraints: Dict = None) -> List[float]:
    """
    修复：不依赖知识库晶格数据，基于元素体积和空间群生成合理晶格参数
    :param similar_structures: 相似结构（仅用于参考原子数，不提取晶格）
    :param atom_types: 原子类型列表
    :param sg_cell_constraints: 空间群晶胞约束
    :return: 合理的晶格参数 [a, b, c, alpha, beta, gamma]
    """
    sg_cell_constraints = sg_cell_constraints or {'a=b=c': True, 'α=β=γ=90°': True}
    num_atoms = len(atom_types)

    # 步骤1：计算总目标体积（基于每个原子的合理体积范围）
    total_target_vol = 0.0
    for num in atom_types:
        min_vol, max_vol = CELL_VOLUME_RANGE.get(num, (10, 30))
        avg_vol_per_atom = (min_vol + max_vol) / 2  # 取每个原子的平均合理体积
        total_target_vol += avg_vol_per_atom
    # 额外增加少量余量（适配空间群堆积）
    total_target_vol *= 1.1

    # 步骤2：基于空间群约束，分配初始晶格参数
    a, b, c, alpha, beta, gamma = 0.0, 0.0, 0.0, 90.0, 90.0, 90.0

    if 'a=b≠c' in sg_cell_constraints:  # 六方晶系（如空间群194）
        # 六方晶系：a=b，γ=120°，体积公式：V = a² * c * sin(120°)
        sin_gamma = np.sin(np.radians(120.0))
        # 先分配a=b的初始值（假设c≈a*1.5，可调整）
        a = (total_target_vol / (1.5 * sin_gamma)) ** (1/3)
        b = a
        c = a * 1.5
        gamma = 120.0

    elif 'a=b=c' in sg_cell_constraints:  # 立方晶系（如空间群225）
        # 立方晶系：a=b=c，体积公式：V = a³
    ...
    elif 'α=β=γ=90°' in sg_cell_constraints:  # 正交/单斜晶系（如空间群62、14）
        # 正交晶系：a≠b≠c，体积公式：V = a*b*c
        # 简单分配：a < b < c，比例约1:1.2:1.5
        ...
        # 三斜晶系：a≠b≠c，α≠β≠γ≠90°，取接近正交的合理值（避免极端角度）
        a = (total_target_vol / (1.2 * 1.5 * 0.98)) ** (1/3)  # 0.98为sin(88°)近似值
        b = a * 1.1
        c = a * 1.4
    ...
    # 步骤3：强化晶格参数上下限约束（避免极端值）
    min_lattice_length = 2.0  # 最小晶胞边长（Å）
    max_lattice_length = 50.0  # 最大晶胞边长（Å）
...
def generate_graph_from_latent(z: torch.Tensor, atom_types: List[int], similar_structures: List[Dict]) -> dgl.DGLGraph:
    ...
    # 添加自环（RGCN必需，避免节点入度为0）
    g = dgl.add_self_loop(g)

    g.graph_attr = {}
    g.ndata['feat'] = torch.zeros(num_atoms, NODE_FEAT_DIM)
    g.edata['feat'] = torch.zeros(g.num_edges(), EDGE_FEAT_DIM)  # 适配自环的边特征
...
# ===================== 10. 核心生成函数 =====================
def generate_valid_structures(formula: str, num_required: int = 50) -> Tuple[List[dgl.DGLGraph], List[float], List[np.ndarray], List[float], List[List[float]], List[Dict]]:
    ...
    # 确保返回足够的有效结构（若不足，补充部分较优结构）
    while len(valid_candidates) < num_required and len(particles) > 0:
        p = sorted(particles, key=lambda x: x.best_fitness)[len(valid_candidates)]
        sg_info = get_space_group_info()
        lattice_params = generate_lattice_params(similar_structs, atom_types, sg_info['cell_constraints'])
        lattice = Lattice.from_parameters(*lattice_params)
        g = generate_graph_from_latent(p.best_position, atom_types, similar_structs)
...
def main():
    ...
    try:
        ...
        print("\n===== 生成完成 =====")
        print(f"✅ 50个候选结构已保存到: {GENERATED_CIF_DIR / formula}")
